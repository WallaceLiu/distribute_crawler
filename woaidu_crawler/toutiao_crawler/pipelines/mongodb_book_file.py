...
from woaidu_crawler.woaidu_crawler.utils.select_result import list_first_item
from woaidu_crawler.woaidu_crawler.pipelines.file import FilePipeline, FSFilesStore, FileException
...
class NofilesDrop(DropItem):
    ...
    def __str__(self):  ##### for usage: print(e)
        print()
        self.style.ERROR("DROP(NofilesDrop):" + self.original_url)
...
class MongodbBookFilesStore(FSFilesStore):
    ...
    def __init__(self, shard_server, shard_port, shard_db, shard_gridfs_collection):
        ...
        try:
            ...
            print()
            self.style.ERROR("ERROR(MongodbBookFilesStore): %s" % (str(e),))
            traceback.print_exc()
